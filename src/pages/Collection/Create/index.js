  ...
  const handleCreate = async () => {
    ...
    try {
      const tx = await createNFTContract(
        ...
        name,
        symbol,
        process.env.REACT_APP_PLATFORM_FEE,
        account
      )
            ...
            clipImage(img, x, y, size, size, async logodata => {
              try {
                ...
                if(resultsavedata.data.status == "success"){
                  ...
                  toast('error', 'Collection created but data saving faild.')
                  setCreating(false)
                }
              } catch (e) {
                setCreating(false)
      ...
      <div className={styles.inner}>
        ...
        {isRegister && (
          <div className={styles.inputGroup}>
            <div className={styles.inputTitle}>
              Fee Recipient *&nbsp;
              <BootstrapTooltip
                title="The nominated Ethereum Metamask wallet address to receive royalties from each sale in this collection."
                placement="top"
              >
                <HelpOutlineIcon/>
              </BootstrapTooltip>
        ...
        {!isRegister && (
          <div className={styles.fee}>
            <InfoIcon/>
            &nbsp;0.004ETHs are charged to create a new collection.
          </div>
        )}
      </div>
      {renderMenu}
